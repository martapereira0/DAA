...
bool avaliar(int value,int arr[], int N,int k){
    int c = 0; // contar o nº de partições necessárias 
    int soma = 0; //somar nºs para definir cada partição
    
    for(int i=0;i<N;i++){
        soma = soma + arr[i];
        if(arr[i] > value) return false; // é impossível fazer uma partição com um número que é maior que o value
        if (soma > value){
...
int bsearch(int low,int high,int arr[], int N,int k){
    while (low < high){
        ...
        if (avaliar(middle,arr,N,k)==true) { // se para aquele valor é possível fazer as k partições, então para os valores superiores a ele também é, logo interessa-nos avaliar para os números inferiores
            high = middle;
        }
        else low = middle + 1;
    }

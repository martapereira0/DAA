...
int procurar(int high,int low, int value,int somas[],int N){
    ...
    if(low == 0 || (low == N-1 && somas[low] < value)){  // precisamos de fazer somas[low] < value caso estejamos com um caso em qque o valor se encontra entre o ultimo valor e o penultimo
...
int main(){
    int N,M,p;
    cin >> N;
    int arr[N]; // array p/ guardar os valores dados com input

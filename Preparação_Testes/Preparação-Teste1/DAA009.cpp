...
int main(){
    string in; // ler o input como um todo
    vector<vector<int>> vec(26,vector<int>(3,0));// vetor com sub-vetores em que em cada sub-vetor na 1ª posição está o valor ascii da letra, na 2ª a frequencia, 3ª o índice em que essa letra surgiu pela 1ª vez

    for(int i=0;i<26;i++){
        vec[i][0]='A'+i; //colocar o valor ascii de cada letra do alfabeto
    }
    cin >> in;
    int len = in.length();
    for(int i=0;i<len;i++){
        //guardar momentaneamente cada caracter do input
        int posi = in[i] - 'A'; // diz a letra que estamos a avaliar da string
        if (vec[posi][1] == 0){
